...
"""
import rospy
from dmp_training import RecordPoseStampedFromPlayMotion, LearnFromRobotTrajectory
...
if __name__ == '__main__':
    ...
    rt = gE.robotTrajectoryFromPlanPoseBased(plan, ['right_arm'], downsample_freq=5)
...
#     robot_traj = gE.robotTrajectoryFromPlan(plan, joint_names)
    ...
    lfrt = LearnFromRobotTrajectory()
    lfrt.start_learn(rt, gesture_name, joint_names, "wave_from_ps_dmp2")
    motion_data_dmp2 = lfrt.stop_learn()
    rospy.loginfo("motion_data_dmp2:" + str(motion_data_dmp2) + "\n end of motion_data_dmp2")
    
    gesture_dict = gG.loadGestureFromBagJointStatesAndRemoveJerkiness("wave_from_ps_dmp2.bag", joint_names, 1)
    copy_gest_dict = copy.deepcopy(gesture_dict)
    del copy_gest_dict['computed_dmp']
    rospy.loginfo("Now we got a DMP and a gesture dictionary containing it, looks like (omitting computed_dmp): " + str(copy_gest_dict))
