...
#     rospy.loginfo("Motion dict is: " + str(motiondata))
    ...
    gesture_dict = gG.loadGestureFromBagEndEffector("random_gesture14.bag", ['/teleop_right_hand_pose'], ['right_arm'])
    ...
    init_time = time.time()
    plan = gG.getPlan(curr_eef_pose_list, gesture_dict["final_pose"], tau=(gesture_dict["duration"]*2), dt=0.02)
    rospy.loginfo("It took " + str(time.time() - init_time ) + "s to get the plan.")
    

    rt = gE.robotTrajectoryFromPlanPoseBased(plan, ['right_arm'], downsample_freq=2)
